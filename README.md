...
建立 D1 `asia-market-intelligence-db`，Pages binding 必須命名 `DB`，依序執行 `migrations/0001_create_page_views.sql` 與 `migrations/0002_add_page_view_created_at.sql`。`wrangler.toml` 刻意不填虛構 `database_id`。API 以 `reports.json` 的日期型唯一 `id` 為統計鍵，並相容既有正式報告路徑；使用參數化 SQL、同源與 body 限制，Sample、首頁、404、預覽、本機與明顯爬蟲不增加正式數。瀏覽器 localStorage 對同一報告只做 24 小時基本去重，D1 才是總數來源。公開數字是近似閱讀次數，不宣稱精準唯一訪客。
