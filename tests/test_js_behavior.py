...
class JavaScriptBehaviorTests(unittest.TestCase):
    def test_frontend_and_api_behavior(self):
        ...
        version=subprocess.run([node,'--version'],capture_output=True,text=True,encoding='utf-8').stdout.strip().lstrip('v').split('.')[0]
        if not version.isdigit() or int(version)<18: self.skipTest('需要 Node.js 18 或更新版本')
        result=subprocess.run([node,str(ROOT/'tests/test_frontend_behavior.mjs')],cwd=ROOT,capture_output=True,text=True,encoding='utf-8')
